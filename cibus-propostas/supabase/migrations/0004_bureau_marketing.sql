-- Slide do Bureau de Marketing Cibus: conteúdo editável por proposta

alter table proposals add column if not exists bureau jsonb not null default '{}'::jsonb;
...
begin
  insert into proposals as t (
    ...
    roi, closing, sections, bureau, created_at, updated_at
  ) values (
    ...
    p->'roi', p->'closing', p->'sections', coalesce(p->'bureau', '{}'::jsonb), coalesce((p->>'created_at')::timestamptz, now()), now()
  )
  on conflict (id) do update set
    ...
    closing = excluded.closing, sections = excluded.sections, bureau = excluded.bureau, updated_at = now();
