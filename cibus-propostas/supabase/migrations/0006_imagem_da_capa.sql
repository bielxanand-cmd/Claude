-- Ajuste da imagem da capa (proporção e modo: automático, preencher, imagem inteira)

alter table proposals add column if not exists cover_layout jsonb not null default '{}'::jsonb;
...
begin
  insert into proposals as t (
    ...
    executive_id, executive, cover_title, cover_subtitle, cover_image, cover_layout, cases_title, cases_subtitle,
    ...
    p->>'cover_title', p->>'cover_subtitle', p->>'cover_image', coalesce(p->'cover_layout', '{}'::jsonb), p->>'cases_title', p->>'cases_subtitle',
  ...
  on conflict (id) do update set
    ...
    cover_subtitle = excluded.cover_subtitle, cover_image = excluded.cover_image, cover_layout = excluded.cover_layout,
