-- Precificação por posto: mensalidade por posto (monthly_price) e implantação
-- escalonada (1º posto + cada posto adicional). A tabela proposal_modules passa
-- a guardar a lista "O que está incluso" (preços zerados).

alter table proposal_investments add column if not exists implementation_first numeric(12,2) not null default 6000;
alter table proposal_investments add column if not exists implementation_additional numeric(12,2) not null default 600;

update settings set data = jsonb_set(
  jsonb_set(jsonb_set(data, '{defaults,monthlyPrice}', '540'), '{defaults,implementationFirst}', '6000'),
  '{defaults,implementationAdditional}', '600')
where id = 1 and not (data->'defaults' ? 'monthlyPrice');
